...
async function createDelitoCiudadano(data) {
  ...
  async function createCiudadano(data) {
    ...
    async function updateBanda(data) {
      ...
      async function getCedulaExpediente(data){
        try {
          const [results, fields] = await conn.execute('SELECT ciu.cedula FROM ciudadano as ciu INNER JOIN delitos_ciudadanos as delc on ciu.id_ciudadano = delc.id_ciudadano WHERE delc.expediente = ?', [data]);
      ...
      module.exports = {createCiudadano, ciudadanoExist, expedienteExist, getCiudadano, createDelitoCiudadano, getDelitosCiudadano, getCedulaExpediente}
