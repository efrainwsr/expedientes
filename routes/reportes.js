...
const queries = require('../queries/reportes.js');
const jwt = require("jsonwebtoken");


router.post('/getTotalDetOrg', async (req, res) => {
    
    const result = await queries.getTotalDetOrg(req.body.org.id_organismo, req.body.rangoFechas);
    res.json({
        //error: false,
        data: result
    })
...
router.post('/getAllDet', async (req, res) => {
    console.log("5. llegamos a routes getAllDet", req.body)
    const result = await queries.getAllDet(req.body.rangoFechas);
    res.json({
        //error: false,
        data: result
    })
