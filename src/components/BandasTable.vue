<template>
    <div class="card">
        <h5>Bandas Criminales</h5>
        <DataTable size="small" v-model:filters="filters" :value="bandas" paginator showGridlines :rows="10" dataKey="id_organismo"
            filterDisplay="menu" :loading="loading"
            :globalFilterFields="['nombre', 'descripcion', 'zona', 'activo']">
            ...
            <template #empty> No se encontraron bandas. </template>
            <template #loading> Cargando datos de bandas. Por favor, espere. </template>
            ...
            <Column field="descripcion" header="Descripcion" style="min-width: 12rem">
                ...
                <template #filter="{ filterModel }">
                    <InputText v-model="filterModel.value" type="text" placeholder="" />
                </template>
            </Column>

            <Column field="zona" header="Zona de operacion" style="min-width: 12rem">
                <template #body="{ data }">
                    {{ data.zona.length > 50 ? data.zona.substring(0, 50) + '...' : data.zona }}
                </template>
                <template #filter="{ filterModel }">
                    <InputText v-model="filterModel.value" type="text" placeholder="Buscar por zona" />
                </template>
            </Column>
            ...
            <Column  field="accion" header="Accion"  bodyClass="text-center" style="min-width: 8rem">
                <template #body="{ data }">
                    <i  @click="cargarDatosBandas(data)"  class="edit-button pi pi-file-edit text-yellow-500" ></i>
                </template>
            </Column>
...
const emit = defineEmits(['cargar-banda']);
const bandas = ref(); //variable que guarda todas las bandas 
const filters = ref();
const loading = ref(false);


// emitir evento para abrir el modal y cargar los datos de la banda seleccionada
const cargarDatosBandas = (data) => {
    emit('cargar-banda', data); // emitir el evento con los datos de la banda a la vista padre
};


onMounted(() => {
    //loading.value = true;
    getAllBandas().then((data) => {
        bandas.value = data.data; // Accede a la propiedad 'data' de tu JSON
        //loading.value = false;
    });
});
...
const initFilters = () => {
    filters.value = {
        ...
        zona: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] },
...
const refreshBandas = async () => {
    const data = await getAllBandas();
    bandas.value = data.data;
};
